    ...
    def validate(self, data):
        ...
        if is_sale:
            ...
            if sale_price is None or sale_price <= 0:
                raise serializers.ValidationError({"sale_price": "sale_pricesms 0보다 커야 합니다."})
            else:
                # 세일이 아니면 sale_price는 아예 없어야 함 (자동 무시하거나 경고)
                if sale_price and sale_price > 0:
                    raise serializers.ValidationError({"sale_price": "is_sale이 False이면 sale_price를 지정할 수 없습니다."})
